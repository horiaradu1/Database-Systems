...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["quiz_id"]))){
        ...
        $id_test = trim($_POST["quiz_id"]);
        $sql = "SELECT Quiz_ID FROM Quiz WHERE Quiz_ID = '$id_test' ";
        $result = mysqli_query($link, $sql);
        if(mysqli_num_rows($result) < 1){
            $quiz_err = "This quiz is does not exist";
        }else{
            $query_check = "SELECT * FROM Quiz WHERE Quiz_ID = '$id_test'";
            $result_check = mysqli_query($link, $query_check);
            $row_check = mysqli_fetch_array($result_check);
            $available = $row_check['Quiz_Available'];
            if($available != 1 | $available != "1"){
                $quiz_err = "Sorry, the quiz is unavailable, try another one: ";
            }else{
                $quiz_id = $id_test;
                $_SESSION["quiz_id"] = $quiz_id;
                header("location: takingquiz.php");
            }
        }
    }
}
...
<body>
    <div class="page-header">
        ...
        <h2>Below are all the available quizzes</h2>
        <br>
        <h4>Enter the number of the quiz you want to TAKE in the box below</h4>
    </div>
    ...
    <p>
        <div class="form-group <?php echo (!empty($quiz_err)) ? 'has-error' : ''; ?>">
            ...
            <br>
            <input type="submit" class="btn btn-primary" value="Take quiz" style='font-size:140%;'>
        </div>
    </p>
    </form>
    ...
    while($row_display = mysqli_fetch_array($result)){   // Creates a loop to loop through results
        if($row_display['Quiz_Available'] == 1){
        ...
        $author_id = $row_display['Quiz_Author_ID'];
        ...
        echo "<font size='+1'><b>QUIZ: </font> </b> | Number: <b>" . $row_display['Quiz_ID'] . "</b> | Name: " . $row_display['Quiz_Name'] . " | Author: " . $author_name . " | Available: " . $available_string . " | Duration: " . $row_display['Quiz_Duration'] . " minutes |<br>";
